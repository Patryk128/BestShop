...
const _package = $('[data-id="package"]');
...
function handleDocument(event) {
    if (!packageSelect.contains(event.target)) selectDropdown.classList.remove("open");
}
function handleDropdown(event) {
    ...
    if (selectedElement.tagName.toUpperCase() === "LI") {
        ...
        _package.setAttribute("data-price", packagePrice);
...
function updatePrice() {
    ...
    products.querySelector(".item__calc").innerText = `${productsQty} * $${productsPrice}`;
    ...
    orders.querySelector(".item__calc").innerText = `${ordersQty} * $${ordersPrice}`;
    orders.querySelector(".item__price").innerText = `$${ordersCost}`;
    totalPrice += ordersCost;
    const packageSinglePrice = +_package.getAttribute("data-price");
    _package.querySelector(".item__calc").innerText = packageSelect.innerText;
    _package.querySelector(".item__price").innerText = `$${packageSinglePrice}`;
    totalPrice += packageSinglePrice;
    let accountingPrice;
    if (accountingCheckbox.checked) accountingPrice = +accountingCheckbox.getAttribute("data-price");
    else accountingPrice = 0;
    accounting.querySelector(".item__price").innerText = `$${accountingPrice}`;
    totalPrice += accountingPrice;
    let terminalPrice;
    if (terminalCheckbox.checked) terminalPrice = +terminalCheckbox.getAttribute("data-price");
    else terminalPrice = 0;
...
},{}]},["hbj89","igcvL"], "igcvL", "parcelRequire010e")
